...
**File:** `compiler.clj`, `engine.cljc` | **Impact:** Reduced per-fact dispatch overhead for multi-node types

When a fact type has N alpha nodes, replace N independent `alpha-activate` dispatches with a single `FusedAlphaNode` that evaluates all activations in one per-fact-then-per-node pass, accumulates elements per original node, then batch-dispatches to beta children.
Fusion is applied at `get-alphas-fn` construction time (not in `alpha-roots`) to preserve durability serialization compatibility.
...
== Proposed Optimizations (Phase 2-3)

The following optimizations have been identified but not yet implemented.

=== 2.2 — Alpha Discrimination Tree

**Files:** `compiler.clj`, `engine.cljc` | **Phase:** 2

Currently each `AlphaNode` independently evaluates its condition against every fact of the matching type.
When many rules share overlapping conditions on the same type, this duplicates work.

An **alpha discrimination tree** (also called an alpha discrimination network) shares common prefixes:
...
1. Analyze alpha-node conditions at compile time to identify shared field access patterns
2. Build a trie where internal nodes test field values and leaves are alpha nodes
3. Traverse the trie once per fact instead of evaluating each alpha node independently

**Expected benefit:** Reduces per-fact alpha evaluation from O(N~nodes~) to O(depth~trie~). Most impactful when many rules pattern-match on the same fact type with overlapping field constraints.

=== 2.4 — Beta Sub-Indexing for ExpressionJoinNode

**Files:** `compiler.clj`, `engine.cljc` | **Phase:** 2

`ExpressionJoinNode` uses a `join-filter-fn` that is applied to every (token, element) pair.
When the filter contains an equality sub-expression (e.g., `(= (:id ?order) (:order-id fact))`), this equality can be extracted at compile time and used as a hash index, reducing the pairs evaluated by the filter.

....
  Current ExpressionJoinNode:
    tokens × elements → filter each pair → O(T × E)

  With sub-indexing:
    tokens × elements[matching-key] → filter each pair → O(T × E')
    where E' ≪ E when key is selective
....

**Approach:**

1. At compile time, analyze `join-filter-fn` for extractable equality expressions
2. Generate a compound key from these expressions
3. Index elements by this key in alpha-memory
4. At join time, compute the key from the token and look up only matching elements
5. Apply the remaining non-equality portion of the filter to the reduced set
...
....
  Eager Rete (current):
    insert fact → propagate through ALL matching paths immediately

  Lazy/PHREAK:
    insert fact → mark paths as dirty
    fire-rules → walk dirty paths top-down, evaluate only what's needed
....

**Key ideas:**

* Maintain a dirty/stale flag per node segment
* On `fire-rules`, traverse from production nodes backward, pulling data through the network
* Skip evaluation for entire sub-networks that cannot produce new activations
* Particularly beneficial for sessions with many rules but few active at any time

**Considerations:**

* Significant architectural change — touches session model, memory, and all node types
* Must preserve correctness of truth maintenance under lazy evaluation
* Negation and accumulation semantics need careful handling

=== 3.2 — Collection-Oriented Batch Joins

**Phase:** 3

Currently, join nodes process one token or element at a time (even though the API accepts collections).
A batch join processes sets of tokens against sets of elements in a single operation, enabling:

* Vectorized hash lookups
* Reduced per-item overhead (function call, protocol dispatch)
* Potential for SIMD-friendly data layouts

....
  Current:
    for token in tokens:
      for element in elements:
        if join-matches?(token, element):
          emit new-token

  Batch:
    group tokens by join-key
    group elements by join-key
    for each shared key:
      cross-product(token-group, element-group)
....

=== 3.3 — Incremental Truth Maintenance via Delta Propagation
...
|3.1
|Lazy/demand-driven evaluation (PHREAK)
|`engine.cljc`, `memory.cljc`
|Proposed

|3.2
|Collection-oriented batch joins
|`engine.cljc`
|Proposed
