...
|**Partial** -- implemented at RootJoinNode → HashJoinNode boundary only
...
**Partial exception (Phase 3.1 — implemented):** `RootJoinNode` defers token creation when all its `HashJoinNode` children have `:left-parent-id` set and their alpha-memory is empty.
...
|`RootJoinNode` deferral
|**Done**
|Defers token creation when children have `:left-parent-id` and empty alpha-memory (`engine.cljc` ~line 854)

|`HashJoinNode` demand-pull
|**Done**
|`right-activate` fetches parent elements on first arrival when `left-parent-id` is set (`engine.cljc` ~line 950)

|Compiler `:left-parent-id` assignment
|**Done**
|Emitted for direct single-parent children of `RootJoinNode` (`compiler.clj` ~line 1977)

|`ExpressionJoinNode` demand-pull
|**Partial**
|Field `:left-parent-id` present in record but demand-pull logic not wired into `right-activate`

|`NegationWithJoinFilterNode` demand-pull
|**Not started**
|No `:left-parent-id` field

|`AccumulateWithJoinFilterNode` demand-pull
|**Not started**
|No `:left-parent-id` field

|Delta protocols (`IDeltaReceive`, `IDeltaEvaluate`, `IDeltaTransport`)
|**Not started**
|Full PHREAK phase -- not yet designed or implemented

|Segment / dirty-flag infrastructure
|**Not started**
|No segment concept exists yet

|`fire-rules*` pull phase
|**Not started**
|Loop is still pure eager/eager-flush
|===

=== Phase A: Infrastructure (Proposed)
...
=== Phase B: Node Migration (Proposed)

Migrate nodes one at a time, maintaining backward compatibility through the transition:

. `ExpressionJoinNode` -- complete the demand-pull wiring already started (field present, logic missing)
. `NegationNode` / `NegationWithJoinFilterNode` -- add `:left-parent-id` and match-count tracking
...
=== Phase C: fire-rules Integration (Proposed)
...
=== Phase D: Cleanup & Optimization (Proposed)
