...
PHREAK combines two planned optimizations into a single coherent architecture:

[cols="1,2,2,1"]
|===
|ID |Optimization |Role in PHREAK |Status

|3.1
|Demand-pull token generation
|The **pull model** -- defer token creation at RootJoinNode, generate on demand when elements arrive
|**Done** -- implemented at RootJoinNode → HashJoinNode boundary

|3.2
|Incremental truth maintenance (delta propagation)
|The **delta model** -- propagate only changes, not full re-derivation
|**Done** -- opt-in via `:use-phreak true` in `fire-rules`
|===
...
== Current Architecture: Eager Rete with Partial Demand-Pull
...
**Phase 3.1 (done):** `RootJoinNode` defers token creation when all its `HashJoinNode` children have `:left-parent-id` set and their alpha-memory is empty.
When elements first arrive at such a `HashJoinNode` via `right-activate`, it demand-pulls matching facts from the parent `RootJoinNode` to generate tokens on-demand rather than having received them eagerly.
This optimization is limited to the `RootJoinNode` → `HashJoinNode` boundary; `ExpressionJoinNode`, `NegationWithJoinFilterNode`, and `AccumulateWithJoinFilterNode` do not participate.

**Phase 3.2 (done):** Full PHREAK delta propagation is implemented as an opt-in path.
...
|`RootJoinNode` deferral (3.1)
|**Done**
|Defers token creation when children have `:left-parent-id` and empty alpha-memory (`engine.cljc`)

|`HashJoinNode` demand-pull (3.1)
|**Done**
|`right-activate` fetches parent elements on first arrival when `left-parent-id` is set

|Compiler `:left-parent-id` assignment (3.1)
|**Done**
|Emitted for direct single-parent children of `RootJoinNode` (`compiler.clj`)

|`IDeltaMemory` + `TransientDeltaMemory` (3.2)
|**Done**
|`delta_memory.cljc` — ordered op lists per node; 3 volatiles (elem-ops, tok-ops, dirty)

|`DeltaTransport` (3.2)
|**Done**
|`engine.cljc` — queues insert/retract ops and marks nodes dirty instead of cascading

|`IDeltaEvaluate` + `evaluate-delta` per node (3.2)
|**Done**
|All node types in `engine.cljc`; `default-evaluate-delta` fallback for complex nodes

|`compute-beta-topo-order` (3.2)
|**Done**
|DFS pre-order from beta roots; stored as `:beta-topo-order` in rulebase

|`run-pull-phase!` + `fire-rules-phreak*` (3.2)
|**Done**
|Pull loop converges (re-runs until no dirty nodes); fire loop interleaves pull after each activation

|Test coverage (3.2)
|**Done**
|`opts-fixture` runs all 713 CLJ tests under PHREAK; 0 failures
|===

=== Known Limitations / Future Work

* `ExpressionJoinNode` demand-pull: field `:left-parent-id` present but demand-pull not wired
* `NegationWithJoinFilterNode`, `AccumulateWithJoinFilterNode` demand-pull: not wired
* `AccumulateWithJoinFilterNode.right-retract`: does not use `retract-fn` (TODO)
* PHREAK path is opt-in (`:use-phreak true`); removing the feature flag and making it default is Phase D
* Delta queue structures could be further optimized (ArrayList on CLJ, typed array on CLJS)

=== Phase D: Cleanup & Optimization (Future)

. Remove `:use-phreak` feature flag, make PHREAK the default path
. Optimize delta queue data structures (ArrayList on CLJ, typed array on CLJS)
. Complete demand-pull wiring for `ExpressionJoinNode`, `NegationWithJoinFilterNode`, `AccumulateWithJoinFilterNode`
