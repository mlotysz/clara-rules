...
The `release` build task creates a git tag for the current version and installs the JAR to the local Maven repository (`~/.m2`).

[source,bash]
----
# Tag HEAD with the latest git tag and install to local ~/.m2
...
1. `:version` argument passed on the command line
2. Latest git tag (`git describe --tags --abbrev=0`)

`jar` and `install` also accept an optional `:version` argument via the same mechanism.
