...
Numbered result snapshots are kept in this directory for historical comparison:
...
| `01-oracle.txt`
| Baseline: `com.cerner/clara-rules` 0.24.0 released JAR

| `02-optimizations.txt`
| Fork after Phases 1–3.1: all micro-optimizations (alpha network, beta joins, negation sub-indexing, PHREAK demand-pull)

| `03-phreak.txt`
| Fork after Phase 3.2: full PHREAK delta propagation (delta queues + pull phase, gated behind `:use-phreak true`)
|===
...
# Save fork results to a specific file (e.g. when capturing a new snapshot)
BENCH_OUT=bench/03-phreak.txt bash bench/compare.sh
...
| `BENCH_OUT` | Fork output file path | `results-fork.txt`
| `ORACLE_OUT_FILE` | Oracle baseline file | `01-oracle.txt`
|===
