...
0. Assert all tests are passing and the project builds: `clojure -T:build clean && clojure -T:build javac && clojure -M:test`
...
5. Set the `CLOJARS_USERNAME` and `CLOJARS_PASSWORD` environment variables with your Clojars credentials.
6. Update the version in `build.clj`, then run `clojure -T:build deploy`
